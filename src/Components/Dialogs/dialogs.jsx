...
const Dialogs = (props) => {
  let userList = props.users.map((user) => (
    <DialogItems id={user.id} name={user.name} usersInfo={props.usersInfo} />
  ));
  ...
  let onAddMessage = (event) => {
    if (
      event.messageInputArea === undefined ||
      event.messageInputArea === "" ||
      event.messageInputArea.length > 1000
    ) {
      return;
    } else {
  ...
  return (
    <div className={css.dialogs}>
      <div className={css.dialogsItems}>{userList}</div>
