...
type authReducerAcType = {
  setCurrentUserInfo: (authInfo: any) => {
    type: typeof GET_AUTHORIZATION_INFO;
    authInfo: any;
  };
  ...
  setAuthorization: (authStance: number) => {
  ...
  writeLoginFailedInfo: (serverRespond: any) => {
    type: typeof WRITE_LOGIN_FAILED_INFO;
    payload: any;
  };
...
export const authReducerAC: authReducerAcType = {
  ...
  setAuthorization: (authStance) => ({
    type: SET_AUTHORIZATION,
    switch: authStance,
  }),
  setLoading: (isLoading) => ({ type: SET_ISLOADING, payload: isLoading }),
  writeLoginFailedInfo: (serverRespond) => ({
    type: WRITE_LOGIN_FAILED_INFO,
    payload: serverRespond,
  }),
      ...
      samuraiJsAPI.auth
        ...
        .then((result) => {
          if (result.resultCode === 0) {
            samuraiJsAPI.auth.authMe().then((result) => {
              dispatch(this.setCurrentUserInfo(result));
              dispatch(this.setAuthorization(result.resultCode));
            });
          } else {
  ...
  logoutThunkCreator: function () {
    return (dispatch) => {
      samuraiJsAPI.auth.logout().then((result) => {
        if (result.resultCode === 0) {
